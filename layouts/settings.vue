...
<template>
            ...
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-800 px-6 ring-1 ring-white/10">
                <div class="flex h-16 shrink-0 items-center">
                  <nuxt-link to="/"
                             class="flex h-16 w-16 items-center justify-center focus:outline-none focus:ring-2 focus:ring-inset focus:ring-orange-300">
  ...
  <div class="xl:pl-72">
    ...
    <div
        class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-6 border-b border-white/5  px-4 shadow-sm sm:px-6 lg:px-8">
        ...
        <nav class="flex overflow-x-auto py-4">
          <ul role="list"
            ...
            <li v-for="item in secondaryNavigation" :key="item.name">
              <a :href="item.href" :class="item.current ? 'text-orange-500' : ''">{{ item.name }}</a>
            </li>
          </ul>
        </nav>
      </header>
