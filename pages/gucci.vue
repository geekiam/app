            ...
            <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
              ...
              <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 ring-1 ring-white/10">
                <div class="flex h-16 shrink-0 items-center">
                  <nuxt-img class="h-8 w-auto" src="brand/icon" alt="Your Company" />
                </div>
                <sidebars-wide />
              </div>
            </DialogPanel>
    ...
    <div class="xl:pl-28">
      ...
      <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-6 border-b border-white/5 bg-gray-900 px-4 shadow-sm sm:px-6 lg:px-8">
        <button type="button" class="-m-2.5 p-2.5 text-white xl:hidden" @click="sidebarOpen = true">
          ...
          <Icon name="streamline:interface-setting-menu-parallel-hamburger-circle-navigation-parallel-hamburger-buttonmenu-circle" class="size-5" aria-hidden="true" />
        </button>
      </div>
<div>
  ...
  <TransitionRoot appear :show="show" as="template" enter="transition-all duration-500 ease-in-out transform"
                  enter-from="translate-x-full" enter-to="translate-x-0" leave="transition-all duration-500 ease-in-out transform"
                  leave-from="translate-x-0" leave-to="translate-x-full">
    <section v-show="show" id="content" class="lg:block w-full lg:w-3/4 fixed lg:inset-y-0 inset-0 right-0 border border-solid border-gray-700 p-2 h-full min-w-0 flex-col overflow-y-auto dark:bg-gray-800 bg-gray-100 dark:text-gray-100 z-50">
      <button class="absolute top-2 right-2" @click="show = false">
        <Icon name="material-symbols:close" class="text-white h-6 w-6" />
      </button>
      <p class="text-sm/6 font-semibold text-white">Main</p>
    </section>
  </TransitionRoot>
        ...
        <main class="flex flex-col lg:flex-row min-w-0 flex-1">
          ...
          dark:text-gray-200 text-gray-900">
            <div class="relative flex flex-col overflow-y-auto">
              <p class="text-sm/6 font-semibold text-white">Aside column</p>
              <button class="text-white" @click="show = !show">Button</button>
            </div>
          </aside>
          <section id="content"
